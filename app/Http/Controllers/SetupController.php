...
class SetupController extends Controller
{
    public function seed(Request $request)
    {
        ...
        $secret = config('app.setup_secret', 'midnight-travel-setup-change-me');

        if ($token !== $secret) {
            return response('<h1>Invalid or missing token</h1><p>Use: /setup/seed?token=YOUR_SETUP_SECRET</p>', 403);
        }
        ...
        return response()->view('setup.seed-done')->setStatusCode(200);
    }
}
